...
'''
    ...
    Description:
        dfa_simulator.py is both the DFA simulator as well as the parser for reading
        in the specifications of a DFA from a text file.

    Usage:
        python dfa_simulator.py <filename> <input string>
'''
...
'''
    Parse the input from the Scanner object, and construct data structures necessary for DFA simulation.
'''
def automaton():
    global token
...
'''
    This is where the transition matrix is constructed.
'''
def tfunction():
    global token
...
'''
    The method that simulates the DFA.
'''
...
'''
    Main method. Where the DFA specifications are read in, and the DFA simulation method is called.
'''
def main(filename, test_string):    
    ...
    try:
        ...
        print 'Could not read input file. Program will exit.'
        sys.exit(1)
    #create the token
    token = Token()
    
    #parse the DFA specifications using scanner and token
    automaton()
...
if __name__ == '__main__':
    ...
    if not (len(sys.argv) == 3):
        print 'usage: python dfa_simulator <filename> <test string>'
        sys.exit(0)
